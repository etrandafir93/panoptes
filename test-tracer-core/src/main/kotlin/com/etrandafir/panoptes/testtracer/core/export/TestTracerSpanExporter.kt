...
/**
 * OpenTelemetry [SpanExporter] that writes each span as a line of OTLP-shaped
 * JSON into an NDJSON file. Designed to be paired with `SimpleSpanProcessor` so
 * spans flush to disk on `end()` — no batch loss if the test JVM dies.
 */
class TestTracerSpanExporter(
    private val writer: NdjsonSpanWriter,
    private val serializer: OtlpJsonSerializer = OtlpJsonSerializer(),
) : SpanExporter {
